...
impl LendingPool {
    ...
    /// Enforces a 50% Close Factor (except for small positions <= $1.00).
    pub fn liquidate(env: Env, liquidator: Address, borrower: Address, repay_amount: i128) {
        ...
        // Close factor check (max 50% repayment for positions > $1.00 USD)
        let max_repay = if pos.debt_borrowed <= 10_000_000 {
            pos.debt_borrowed
        } else {
            pos.debt_borrowed.checked_div(2).expect("div zero")
        };
        if repay_amount > max_repay {
            panic!("REPAY_EXCEEDS_CLOSE_FACTOR");
        }
    ...
    /// Atomic swap: receiver transfers collateral XLM to the pool, and pool mints dTOKEN to receiver using current oracle price.
    pub fn swap_collateral_for_debt(env: Env, receiver: Address, xlm_amount: i128) {
        receiver.require_auth();
        if xlm_amount <= 0 { panic!("amount must be positive"); }

        // 1. Transfer XLM from receiver to the pool via transfer_from
        let collateral_addr = get_collateral_addr(&env);
        let collateral = token::Client::new(&env, &collateral_addr);
        collateral.transfer_from(&env.current_contract_address(), &receiver, &env.current_contract_address(), &xlm_amount);

        // 2. Calculate debt to mint using oracle price
        let price = get_xlm_price_usd(&env);
        let dtoken_amount = xlm_amount
            .checked_mul(price).expect("overflow")
            .checked_div(10_000_000).expect("div zero");

        // 3. Mint dTOKEN to receiver
        let debt_addr = get_debt_addr(&env);
        let debt = DebtTokenClient::new(&env, &debt_addr);
        debt.mint(&receiver, &dtoken_amount);
    }

    /// Perform a flash loan of dTOKEN.
    pub fn flash_loan(
        env: Env,
        receiver: Address,
        amount: i128,
        ...
        let callback_args: soroban_sdk::Vec<soroban_sdk::Val> = (
        ...
        env.invoke_contract::<soroban_sdk::Val>(&receiver, &callback_fn, callback_args);

        // 4. Burn loan + fee from receiver
        let total_repay = amount.checked_add(fee).expect("overflow");
        debt.burn(&receiver, &total_repay);
    }
}
